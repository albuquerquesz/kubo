...
## Idle recipe — static pixel pose

When the desired reference keeps the mascot planted, render the authored SVG
pose directly instead of applying a frame lookup or transform to its legs. Keep
the body, root, both leg groups, SVG dimensions, and wrapper untouched so the
character remains pixelated and layout-stable. Blink or other pixel effects may
be driven independently, but must not move the mascot as a whole.
