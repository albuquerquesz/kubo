...
choose a project shape, and continue through representative core-stack selections.
...
- It does not execute a shell command, create files, use the network, or accept arbitrary keyboard
  input.
- The demonstrated path uses this representative configuration:
  `my-kubo-app`, TanStack Router, Elysia, Bun, tRPC, PostgreSQL, Drizzle, Better Auth, GetMonitor, and
  Turborepo.
- The simulation uses the actual prompt labels and selection markers from the CLI, then continues
  through representative backend, runtime, and database selections.
- The prompt sequence uses `│` connectors between stages, matching the CLI tree structure.
- The loop duration is 12 seconds, with a short hold on the completed database selection before
  restarting.
...
2. Reveal the Kubo banner.
3. Type and submit `my-kubo-app`.
4. Show `Select project type` with `Web` selected.
5. Show `Choose web` with `TanStack Router` selected.
6. Show `Select backend` with `Elysia` selected.
7. Show `Select runtime` with `Bun` selected.
8. Show `Select database` with `PostgreSQL` selected.
...
- The animated terminal visual is hidden from assistive technology to avoid repeatedly announcing a
  looping transcript.
- A static screen-reader transcript describes the command and selected stack.
...
- Typed playback state and deterministic 12-second loop.
- CLI-styled command, prompts, and representative stack selections.
...
- [ ] The visual sequence follows the defined Create Path and loops without a long empty hold or duplicated content.
