...
| **Current**      | 3 Lucide icons, `size-14` + `overflow-hidden`, `yPercent 100→0`, scrub **0.45**, stagger **0.12**, start `"top 75%"` / end `"top 35%"`, desktop-only.                                                                                                                                                                                    |
| **Required**     | Mechanics already match Family C core. **Retune ScrollTrigger range** so rise completes across the **same sticky pin window as Family B** (icons lead with stagger inside that scrub), not a short independent 75%→35% window that finishes before sticky scale ends. Keep mask 56px, 3 icons, `ease: "none"`, reduced motion → visible. |
