...
- `kubojs.jsonrc` permite que a CLI detecte e aprimore o projeto depois; mantenha-o se planejar usar `kubojs add`.
...
```json
{
  ...
  "addons": ["<pwa|tauri|electrobun|biome|lefthook|husky|mcp|turborepo|vite-plus|oxlint|opentui|skills|s3-storage|none>"] ,
...
- `apps/api`: quando `--api` não é `none` (não-Convex); concentra a API gerada por domínio em `src/modules`
- `packages/auth`: quando `--auth` não é `none` (não-Convex)
- `packages/db`: quando `--database` e `--orm` são selecionados (não-Convex)
- `packages/storage`: quando o addon `s3-storage` é selecionado
- `packages/backend`: somente backend Convex
- `packages/infra`: somente deploy Cloudflare

## Scripts de desenvolvimento

Os scripts são ajustados conforme o gerenciador de pacotes e se o addon Turborepo ou Vite+ está selecionado.
...
Sem Turborepo ou Vite+ (exemplo para Bun):
