...
kubojs now supports a fully JSON-first workflow for agents and automation:
...
Run kubojs itself as a local MCP server over stdio:
...
This is the best path when an MCP-capable client should scaffold or modify kubojs projects without shelling out to a prompt-driven CLI flow.
...
If you scaffold a project with the `mcp` addon, kubojs itself is now one of the recommended MCP servers. The addon installs it through `add-mcp` with a package runner command so the generated config does not rely on a globally installed CLI:

```bash
npx -y add-mcp@latest "npx -y kubojs@latest mcp"
```

For Bun projects, the generated config uses the equivalent `bunx kubojs@latest mcp` server command inside `add-mcp`. That means MCP-capable agents inside the generated project can talk back to kubojs without any extra global install, alongside other recommended docs, framework, and database MCP servers.

## Use it inside your AI agent

Install the kubojs plugin and your assistant will scaffold and extend projects for you — just ask in plain English, like _"create a fullstack app with Next, Hono, Postgres and Better Auth"_, and it builds a valid stack instead of hand-writing boilerplate.
...
- `/kubojs:new <description>` — scaffold a new project
- `/kubojs:add <addons>` — add addons to an existing one

### Codex

Add the same marketplace from the Codex plugins screen and install **kubojs**, then ask for a project in chat.
...
Any other agent works too, because kubojs is a CLI underneath:

- **MCP-capable agents** (Cursor, VS Code, Gemini CLI, Zed, and more) — wire the server with `npx -y add-mcp@latest "npx -y kubojs@latest mcp"` and pick your agent.
- **CLI-first agents** like [pi](https://pi.dev) — just ask it to scaffold with kubojs and it runs the `kubojs` CLI directly.
...
When `skills` is used with the `evlog` addon, kubojs recommends Evlog's agent skills from `https://www.evlog.dev`: `review-logging-patterns` and `analyze-logs`.
...
kubojs persists structured configuration in `bts.jsonc`, including:
