...
O addon `s3-storage` não possui opções interativas: ele gera diretamente o pacote compartilhado de bucket e as variáveis de ambiente S3-compatible.
...
## Armazenado em `kubojs.jsonrc`

O kubojs persiste configuração estruturada em `kubojs.jsonrc`, incluindo:

- `reproducibleCommand`
- `addonOptions`
- `dbSetupOptions`

Veja [`kubojs.jsonrc`](/docs/kubojs-config) para o formato do arquivo.
