...
The kubojs CLI provides several commands to manage your TypeScript projects.

## `create` (Default Command)

Creates a new kubojs project.
...
Adds addons to an existing kubojs project.
...
Start the kubojs MCP server over stdio. It is launched by an MCP client (Claude Code, Codex, Cursor, opencode, and others), not run directly.
...
Opens the kubojs documentation in your default browser.

```bash
kubojs docs
```

Opens `https://kubojs.dev/docs` in your browser.
...
Opens `https://kubojs.dev/new` where you can configure your stack visually.
