...
- `s3-storage`: Pacote de bucket tipado compatível com AWS S3, Cloudflare R2, MinIO e Backblaze

O addon `s3-storage` gera as variáveis `S3_*` e uma factory para criar o adapter no backend:

```typescript
import { createS3BucketFromEnv } from "@my-project/storage";
...
`S3_ACCESS_KEY_ID` e `S3_SECRET_ACCESS_KEY` devem ser informadas juntas. Sem elas, o SDK pode usar seu provider padrão de credenciais.
