...
export const loadTimelineFromFile = async (filename: string) => {
  ...
  const lengthMs =
    timeline.elements.length > 0 ? timeline.elements[timeline.elements.length - 1].endMs / 1000 : 0;
...
export const calculateBlur = ({ item, localMs }: { item: BackgroundElement; localMs: number }) => {
  const maxBlur = 1;
  const fadeMs = 1000;
...
export const getImagePath = (proj: string, uid: string) => `content/${proj}/images/${uid}.png`;

export const getAudioPath = (proj: string, uid: string) => `content/${proj}/audio/${uid}.mp3`;
