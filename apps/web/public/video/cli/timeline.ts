  ...
  for (let i = 0; i < storyWithDetails.content.length; i++) {
    ...
    for (const word of words) {
      ...
      for (let i = 0; i < word.length; i++) {
        currentEndMs = character_end_times_seconds[currentCharIndex] * 1000 + durationMs;
        currentCharIndex++;
      }
    ...
    if (currentText.trim().length > 0) {
      const textElem: TextElement = {
        startMs: currentStartMs,
        endMs:
          character_end_times_seconds[character_end_times_seconds.length - 1] * 1000 + durationMs,
        text: currentText.trim(),
        position: "center",
        animations: getTextAnimations(),
      };
