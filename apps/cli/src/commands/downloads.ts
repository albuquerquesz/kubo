...
export type DownloadQueryOptions = {
  packageName?: string;
...
export async function getAllTimeDownloads(options?: DownloadQueryOptions): Promise<DownloadReport> {
...
export function formatHelp(usage = "bun run downloads [--json]"): string {
  return [`Usage: ${usage}`, "", `Reports all-time npm downloads for ${PACKAGE_NAME}.`].join("\n");
}
...
export async function runDownloadsCommand(
  ...
  const report = await getAllTimeDownloads(options);
  console.log(parsedArguments.json ? JSON.stringify(report, null, 2) : formatHumanReport(report));
}
